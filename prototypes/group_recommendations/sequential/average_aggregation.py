...
def generate_sequential_recommendations(group, user_ratings, movies, iterations=5, top_k=10):
    ...
    for iteration in range(iterations):
        # Calculate movie scores based on current preferences and satisfaction adjustments
        for movie in movies:
            movie_scores[movie] = 0  # Reset scores per movie per iteration

            # Sum preference scores for each user in the group, applying dynamic weighting
            for user in group:
                user_satisfaction_weight = 1.0  # Default weight

                # Decrease weight for more satisfied users to prioritize least satisfied
                if iteration > 0 and user_satisfaction_history[user][-1] < 0.8:  # Adjust threshold as needed
                    user_satisfaction_weight = 1.5  # Increase weight if user satisfaction was low in the last iteration

                movie_scores[movie] += user_satisfaction_weight * preference_score(user, movie, user_ratings)
